...
vacuum_result vacuum(const vacuum_request& request) {
	...
	for (int attempt = 0; attempt < request.max_tries && !done; ++attempt) {
		...
		const file_id_t before = file_id_of_fd(target_before);
		...
		const file_id_t after = target_after.fd() >= 0
			                            ? file_id_of_path(request.data_path)
			                            : k_invalid_file_id;
		...
		//
		// THE SECOND HALF IS ALSO WHAT CARRIES "nothing there before, nothing
		// there now" (#195). `file_id_equal` refuses to call two failed `stat`s
		// a match - the absence of evidence is not evidence - so the "gone"
		// disjunct, and not the comparison, is what lets a first vacuum on a
		// fresh machine proceed. The outcome is the one the stub gave.
		if (!(file_id_equal(after, before) || after == k_invalid_file_id)) {
			++out.retries;
			continue;
		}
