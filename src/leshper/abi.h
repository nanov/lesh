 ...
 * VALUE, when a verb has more than one: the required ones cannot be forgotten
 * because they have no default and no field to leave unset, and an options value
 * zero-initialized (`{0}` in C, `{}` in C++) means every optional at its default,
 * so a call reads as the configuration it stands for rather than a run of
 * positional NULLs. Each such struct is FROZEN AT BIRTH, the same rule as above
 * applied to structs rather than functions: a field is never appended, because
 * growth already has its one mechanism - a new verb taking a new struct - and a
 * second one, a `sizeof`-tagged version an old caller's struct might be too small
 * for, would tax every call for a case the first already covers for free. This is
 * not the copy-out convention below (`char* out, size_t capacity,
 * size_t* length_out`), which is a different contract for a different problem and
 * is untouched by it.
 *
 * LISTS ARE A POINTER AND A COUNT, NEVER `...`. A real variadic is not
 * `printf`-portable across a binding and would put the calling convention back in
 * the caller's hands, so no verb on this boundary is one. `count == 0` is the
 * empty list, and the pointer is unread when it is; otherwise a NULL pointer is
 * LESH_ERR_INVAL, the same rule a required struct pointer would get.
 *
 * Both rules are the boundary's and not this file's: the sibling header named
 * below states them again, and carries the worked examples, because its verbs are
...
/* ------------------------------------------------------------------------- */
/* The prompt: `ui/prompt/abi.h` (#170)                                       */
/*                                                                            */
/* The `lesh_prompt_*` verbs were declared here through #157 and are declared  */
/* in `src/ui/prompt/abi.h` now - same names, same signatures, same            */
/* constants, and the same frozen-struct and additive-growth rules, restated   */
/* at that header's head. They moved because the engine did: a prompt renders  */
/* SHELL FACTS - cwd, exit status, jobs, time, the git branch - which is host  */
/* knowledge, so the engine is the host's state and the editor takes prompt    */
/* BYTES only (`loop_options::prompt` / `.continuation`, plus a pen). That     */
/* header includes this one, for the status codes and the handle and copy-out  */
/* conventions its verbs follow; the registry still carries the slot those     */
/* verbs resolve through, and this side declares it as a `void*` it never      */
/* dereferences and whose type it does not name. Include both headers to       */
/* configure a prompt; this one alone drives the editor.                       */
/* ------------------------------------------------------------------------- */
