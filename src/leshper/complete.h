...
//
// NOTHING FROM THE EXPANDER, AND THE LINK GRAPH ENFORCES IT. `complete.cpp`
// includes `syntax/` and `substrate/` and no `runtime/` header; the expander
// lives in `lesh_runtime`, which `lesh_leshper` does not link (spec §4.4), so an
// include that reached for it would fail to LINK rather than fail review. That
// is #11's guarantee made structural: `$VAR` in a completion prefix is never
// expanded in v1, because there is no code here that could expand it. The one
// environment read is `$HOME` for a leading `~`, which §6.9 asks for by name;
// see `expand_tilde_for_listing` in the .cpp for why that is not the same door.
//
...
// those four with `pager_kind`: the `ls -F` marker IS the v1 description, and
// the marker decides the trailer. So a candidate here is a `pager_candidate`
// and nothing more, and `lesh_pager_add` takes it apart in the two arguments it
...
using candidate = pager_candidate;
