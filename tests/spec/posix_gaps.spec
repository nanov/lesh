...
--- cd keeps a logical PWD through a symlink
cd /tmp && pwd

--- cd .. is lexical, not physical
cd /tmp && cd .. && pwd
...
--- cd -P resolves a symlink where -L keeps it
...
--- dot-dot follows the mode it was reached in
...
--- a CDPATH match is written to standard output
...
--- an empty CDPATH entry means the current directory and prints nothing
...
--- the last of cd -L and cd -P wins
...
--- a cd that did not happen is status 2
cd /nonexistent 2>/dev/null; echo "status=$?"

--- an illegal cd option is status 2
cd -x 2>/dev/null; echo "status=$?"

--- cd - prints where it went and swaps OLDPWD
...
--- a backslash-quoted delimiter suppresses expansion and still terminates
cat <<\EOT
hello $x `false`
EOT
echo after

--- a partly quoted delimiter has quote removal applied
cat <<E'O'T
body
EOT
echo after

--- a delimiter quoted mid-word with a backslash still terminates
cat <<EO\T
body
EOT
echo after

--- a double-quoted delimiter fragment has quote removal applied
cat <<"EO"T
body
EOT
echo after

--- <<- strips tabs with a backslash-quoted delimiter
cat <<-\EOT
	hello $x
	EOT
echo after

--- two here-documents on one line, in order
cat <<A; cat <<B
first
A
second
...
--- arithmetic as a loop counter
i=0; while [ $i -lt 3 ]; do i=$((i+1)); done; echo $i

--- arithmetic precedence follows C
echo $((2 + 3 * 4)) $(((2 + 3) * 4))

--- arithmetic reads variables without a dollar
i=5; echo $((i * i))

--- arithmetic assignment writes back
i=5; echo $((i += 3)); echo $i

--- an unset variable is zero in arithmetic
echo $((no_such_var + 1))

--- arithmetic bases follow C
echo $((0x1f)) $((010))

--- the colon distinguishes unset from empty
x=; echo "[${x:-colon}] [${x-nocolon}]"

--- assign-default writes the value back
echo ${undef_assign:=written}; echo $undef_assign

--- use-alternate substitutes only when set
x=yes; echo "[${x:+alt}] [${unset_alt:+alt}]"

--- all four trimming forms
p=/a/b/c.txt; echo "${p#*/} ${p##*/} ${p%.*} ${p%%.*}"

--- a default value may contain blanks
echo "${undef_blanks:-a default with spaces}"

--- the positional count
echo $#

--- unquoted star and at behave alike
set -- one two; echo $* ; echo $@

--- quoted at produces one field per parameter
set -- a b; for x in "$@"; do echo "[$x]"; done

--- quoted star joins with the first IFS character
set -- a b; echo "[$*]"

--- shift renumbers the positional parameters
set -- a b c; shift; echo "$1 $2"; echo $#

--- set -f disables pathname expansion
set -f; echo /usr/bin/tru*

--- a function receives positional parameters
f() { echo "$1-$2"; }; f a b

--- return sets the status and leaves the function
f() { return 3; }; f; echo $?

--- return stops execution mid-function
f() { echo in; return; echo never; }; f

--- positional parameters are restored after a call
set -- outer; f() { echo "$1"; }; f inner; echo "$1"

--- a redefinition replaces the previous body
f() { echo one; }; f() { echo two; }; f

--- a function sees and mutates global variables
x=before; f() { x=after; }; f; echo $x

--- a function can be recursive
countdown() { if [ $1 -gt 0 ]; then echo $1; countdown $(($1 - 1)); fi; }; countdown 3

--- PPID is set at startup
[ "$PPID" -gt 1 ] && echo ok

--- PPID is not exported
env | grep -c '^PPID='

--- PPID is an ordinary variable and can be assigned
PPID=42; echo "$PPID"

--- PPID does not change in a subshell
outer=$PPID; inner=$( echo "$PPID" ); [ "$outer" = "$inner" ] && echo same

--- an async command inside a brace group runs in the background
F=/tmp/lesh_spec_fifo_brace; rm -f $F; mkfifo $F; { echo foo >$F & }; cat $F; rm -f $F

--- an async command inside a subshell runs in the background
F=/tmp/lesh_spec_fifo_sub; rm -f $F; mkfifo $F; (echo foo >$F &); cat $F; rm -f $F

--- an async command inside a loop body runs in the background
F=/tmp/lesh_spec_fifo_loop; rm -f $F; mkfifo $F; for i in 1; do echo foo >$F & done; cat $F; rm -f $F

--- a redirection to a FIFO is opened once, not speculatively
F=/tmp/lesh_spec_fifo_once; rm -f $F; mkfifo $F; cat $F & echo foo >$F; wait; rm -f $F

--- ! negates a command's status
! false; echo $?; ! true; echo $?

--- ! negates the whole pipeline, not its first command
! echo hi | grep -q x; echo $?

--- ! in an if condition
if ! false; then echo yes; fi

--- set -e exits the shell rather than ending the list
set -e; while true; do echo r1; false; echo nope; break; done; echo nope3

--- set -e is suppressed in an if condition
set -e; if false; then echo a; fi; echo after

--- set -e is suppressed in a while condition
set -e; while false; do :; done; echo after

--- set -e does not fire on a short-circuited and list
set -e; false && echo a; echo after

--- the short-circuit exemption travels out of a brace group
set -e; { false && echo a; }; echo after

--- the short-circuit exemption travels out of a loop body
set -e; while true; do false && echo a; break; done; echo after

--- the short-circuit exemption stops at a subshell
set -e; (false && echo a); echo after

--- the short-circuit exemption stops at a function call
set -e; f(){ false && echo a; }; f; echo after

--- set -e fires on the last command of an and list
set -e; true && false; echo after

--- set -e fires when every operand of an or list fails
set -e; false || false; echo after

--- set -e does not fire on a negated pipeline
set -e; ! true; echo after

--- set -e fires on a case body
set -e; case x in x) false;; esac; echo after

--- set -e reads the pipeline's status, not the first stage's
set -e; false | true; echo after

--- a single quote inside double quotes is an ordinary character
echo "it's"; echo "'b'"; echo "a'b'c"

--- single and double quoted runs concatenate without losing quotes
echo 'x'"'y'"'z'

--- a tilde inside double quotes is not expanded
echo "~"; echo "~/x"; echo "a~b"

--- a single quote survives an expansion in the same word
x="it's"; echo "[$x]"; echo $x

--- a single quote inside double quotes around a command substitution
echo "a$(echo "b'c")d"

--- a file without a shebang is run as a shell script
D=/tmp/lesh_spec_noexec; rm -r $D 2>/dev/null; mkdir -p $D; printf 'echo "ran: $1"\n' > $D/s; chmod +x $D/s; $D/s hello; rm -r $D

--- ENOEXEC fallback also applies to a command found on PATH
D=/tmp/lesh_spec_noexec2; rm -r $D 2>/dev/null; mkdir -p $D; printf 'echo "ran: $1"\n' > $D/prog; chmod +x $D/prog; PATH=$D:$PATH prog hello; rm -r $D

--- a trap listing can be read back in
...
--- trap with a numeric first operand resets every condition
trap 'echo trapped' 2 QUIT; trap 2 QUIT; trap; echo listed-nothing

--- trap -- lets a command start with a hyphen
trap -- '- x' USR1; trap

--- an ignored trap prints as an empty action
trap '' INT TERM; trap

--- wait with no operands reports zero, not the last job's status
false & wait; echo "wait=$?"

--- wait with a pid reports that job's status
false & wait $!; echo "one=$?"; true & wait $!; echo "two=$?"

--- wait on a pid that is not a child reports 127
wait 99999; echo "wait=$?"

--- exec replaces the shell rather than forking
exec echo replaced; echo notreached

--- exec with no command runs only the redirections
exec; echo reached; exec 2>/dev/null; echo still-here

--- exec redirection outlives the command
F=/tmp/lesh_spec_exec_redir; (exec > $F; echo written); cat $F; rm -f $F

--- an assignment prefixing exec persists because exec is a special builtin
x=1 exec; echo "x=$x"

--- an assignment prefixing exec reaches the new image's environment
LESH_SPEC_EXEC=bar exec sh -c 'echo "got=$LESH_SPEC_EXEC"'

--- exec in a subshell replaces only the subshell
(exec echo inner); echo "after=$?"

--- exec in a pipeline stage replaces only that stage
exec echo piped | cat; echo "after=$?"

--- exec with a command that is not found reports 127 and exits
(exec ./_lesh_no_such_command_); echo "sub=$?"; exec ./_lesh_no_such_command_; echo notreached

--- exec with a file that is not executable reports 126 and exits
exec /etc/hosts; echo notreached

--- a redirection failure on exec exits a non-interactive shell
exec < /_lesh_no_such_file_; echo notreached

--- command exec demotes the special builtin so a redirection failure is survivable
command exec < /_lesh_no_such_file_; status=$?; echo "survived=$status"

--- exec keeps the fd it opened inside a grouping
{ exec 4>&3; } 3>&2; echo grouped >&4; echo "after=$?"

--- exec preserves the process id
exec sh -c "[ \$\$ -eq $$ ] && echo same-pid"

--- an ignored signal stays ignored across exec
trap '' INT; exec sh -c 'kill -INT $$; echo survived'

--- the EXIT trap runs when exec fails
...
--- a redirection on a grouping closes the fd it opened
{ :; } 3>&2; echo foo >&3; echo "status=$?"

--- a redirection on a loop closes the fd it opened
for i in 1; do :; done 3>&2; echo x >&3; echo "status=$?"

--- a redirection on a function call closes the fd it opened
f() { :; }; f 3>&2; echo x >&3; echo "status=$?"

--- a redirection on eval closes the fd it opened
eval : 3>&2; echo x >&3; echo "status=$?"

--- a redirection on a builtin closes the fd it opened
echo hi 3>&2; echo x >&3; echo "status=$?"

--- a nested grouping closes only the fd it opened
{ { :; } 3>&2; echo mid >&3; } 4>&2; echo out >&3; echo "status=$?"

--- a displaced descriptor is put back rather than left closed
exec 3>&1; { echo inner >&3; } 3>&- 2>/dev/null; echo outer >&3

--- the saved copy of a descriptor is out of the way of later redirections
exec 3>&1; { echo inner >&3; } 3>&2 4>&3 2>/dev/null; echo outer >&3

--- a redirection failure reports two
echo x < /_lesh_no_such_file_; echo "status=$?"

--- a redirection failure on a compound command reports two
{ :; } < /_lesh_no_such_file_; echo "status=$?"

--- a redirection failure on an external command reports two
/bin/echo x < /_lesh_no_such_file_; echo "status=$?"

--- a redirection failure on a special builtin exits a non-interactive shell
: < /_lesh_no_such_file_; echo notreached

--- command demotes a special builtin so a redirection failure is survivable
command : < /_lesh_no_such_file_; echo "survived=$?"

--- a redirection failure on a pipeline stage reports two
true | echo x < /_lesh_no_such_file_; echo "status=$?"

--- closing a descriptor that was never open is not an error
echo a 5>&-; echo "out=$?"; echo b 5<&-; echo "in=$?"

--- closing stdout makes a builtin that writes fail
echo x >&-; echo "status=$?"

--- a descriptor closed for a command is closed only for that command
exec 3>&2; { echo a >&3; } 3>&- 2>/dev/null; echo "inner=$?"; echo b >&3; echo "outer=$?"

--- a command that is only redirections performs them
rm -f /tmp/lesh_spec_bare; >/tmp/lesh_spec_bare; echo "st=$?"; test -f /tmp/lesh_spec_bare && echo created; rm -f /tmp/lesh_spec_bare

--- a failing redirection with no command name reports two
< /_lesh_no_such_file_; echo "status=$?"

--- an assignment is skipped when its redirection fails
...
--- noclobber is overridden by the pipe form
...
--- getopts reports the end of the options with a question mark
getopts a x -a; getopts a x -a; echo "st=$? [$x]"

--- a lone hyphen is an operand and a double hyphen ends the options
getopts '' x -; echo "hyphen st=$? [$OPTIND]"; OPTIND=1; getopts ab x -a -- -b; echo "1[$x]"; getopts ab x -a -- -b; echo "2 st=$? [$OPTIND]"

--- getopts leaves OPTIND at the first operand
...
--- an unknown option is a question mark that keeps the loop going
...
--- a leading colon puts the offending letter in OPTARG and prints nothing
...
--- a missing option argument is a colon under the colon form
...
--- a missing option argument is a question mark otherwise
...
--- the colon in an optstring marks an argument and is never an option letter
getopts a:b v -: 2>/dev/null; echo "st=$? [$v]"

--- option letters may be digits
...
--- OPTIND and OPTARG are not exported
getopts a: o -a arg; getopts a: o -a arg; sh -c 'echo ${OPTIND-unset} ${OPTARG-unset}'

--- getopts is a regular builtin, so a usage error does not exit the shell
...
--- test follows the argument-count rules for zero through four arguments
test; echo $?; test ''; echo $?; test -n; echo $?; x=-n; test "$x"; echo $?; test ! -n; echo $?; test = = =; echo $?; test ! ! ''; echo $?; test ! 1 = 2; echo $?

--- test compares integers numerically and reports a non-integer
test 1 -eq 1; echo $?; test 10 -gt 9; echo $?; test 007 -eq 7; echo $?; test x -eq 1 2>/dev/null; echo $?

--- test file predicates answer about the file rather than succeeding
test -d /; echo $?; test -f /; echo $?; test -s /nonexistent; echo $?; test -e /etc/hosts; echo $?; test -h /nonexistent; echo $?

--- test combines expressions with -a, -o and parentheses
test 1 = 1 -a 2 = 2; echo $?; test 1 = 2 -o 2 = 2; echo $?; test ! \( 1 = 1 \); echo $?; test -z '' -a -n x -o -n ''; echo $?; test \( 1 = 2 \) -o \( 3 = 3 \); echo $?

--- the bracket form is the same builtin and requires its closing bracket
[ 1 = 1 ]; echo $?; [ 1 = 2 ]; echo $?; [ 1 = 1 2>/dev/null; echo $?; [ ! ]; echo $?

--- readonly with no operands lists in re-inputtable form
readonly r=1; readonly; readonly u; readonly -p

--- readonly takes -- and marks a name without assigning to it
b=B; readonly -- a=A b c=C; echo $a $b $c; readonly

--- assigning to a readonly variable exits a non-interactive shell
readonly a=1; echo $a; a=2; echo not reached

--- unset refuses a readonly variable, and unset is a special builtin
readonly a=1; unset a; echo not reached

--- exporting a readonly variable is allowed and assigning to it is not
readonly a=1; export a; export a=2; echo not reached

--- a refused assignment inside a parameter expansion is fatal too
readonly x; : ${x=1}; echo not reached

--- a refused assignment inside arithmetic is fatal too
readonly x; echo $((x=1)); echo not reached

--- a readonly name with no value is still unset
...
--- export marks a name without creating the variable
export xa; echo "[${xa-unset}][${xa:-empty}]"; xa=1; echo "[${xa-unset}]"

--- a name marked for export but unset is in no child's environment
export xb; /usr/bin/env | /usr/bin/grep -c '^xb='; sh -c 'echo "[${xb-unset}]"'

--- assigning after export exports the value the assignment gave it
export xc; sh -c 'echo "[${xc-unset}]"'; xc=1; sh -c 'echo "[${xc-unset}]"'

--- export -p prints a marked-but-unset name bare, so re-input does not create it
export xd; export -p | /usr/bin/grep '^export xd'

--- an export -p listing round-trips a marked-but-unset name
export xe; e=$(export -p | /usr/bin/grep '^export xe'); unset xe; eval "$e"; echo "[${xe-unset}]"

--- an operand that field-splits marks every name it yields rather than emptying them
n='xf xg'; export $n; echo "[${xf-unset}][${xg-unset}]"; export -p | /usr/bin/grep -c '^export xf$'

--- export NAME=VALUE still assigns and still exports
export xh=1; echo "$xh"; sh -c 'echo "[${xh-unset}]"'; export -p | /usr/bin/grep '^export xh='

--- unset drops the export mark along with the variable
export xi=1; unset xi; xi=2; sh -c 'echo "[${xi-unset}]"'

--- a readonly name with no value still refuses a later assignment
readonly xj; echo "[${xj-unset}]"; xj=1; echo not reached

--- read reports a readonly target and, being regular, does not exit the shell
readonly v; echo hi | { read v 2>/dev/null; echo "st=$?"; }; echo reached

--- getopts fails rather than ignore a readonly OPTIND
...
--- a command substitution in an EXTERNAL stage reads the pipe
echo a | /bin/echo $(cat)

--- a command substitution in a FUNCTION stage's arguments reads the pipe
f() { echo "[$1]"; }; echo a | f $(cat)

--- a command substitution inside arithmetic in a stage reads the pipe
echo 3 | echo $(( $(cat) + 1 ))

--- a command substitution in a MIDDLE stage reads its own pipe
printf 'm\n' | echo $(cat) | tr m M

--- a command substitution in a stage may be a pipeline itself
echo a | echo $(cat | tr a-z A-Z)

--- a stage's words are expanded BEFORE its redirections are performed
F=/tmp/lesh_spec_p50_f; echo FILE > $F; echo PIPE | echo $(cat) < $F; rm -f $F

--- a fatal expansion error in a stage ends the stage, not the shell
echo a | echo ${x?bad}; echo "rc=$?"

--- a stage with no command name still performs its redirections
F=/tmp/lesh_spec_p50_out; rm -f $F; echo a | > $F; echo "rc=$?"; ls $F; rm -f $F

--- a stage with no command name reports its last command substitution
...
--- exit refuses an operand that is not a number
...
--- an exit status is taken modulo 256 rather than refused
( exit 256 ) 2>/dev/null; echo "a=$?"
( exit 300 ) 2>/dev/null; echo "b=$?"
( exit 255 ) 2>/dev/null; echo "c=$?"

--- shift refuses an operand that is not a number
...
--- return refuses an operand that is not a number
...
--- wait refuses a pid operand that is not a number
...
--- unalias takes its operand after a `--` separator
...
--- a positional parameter index too large to be one is unset rather than wrapped
...
--- a digit run too large to be a file descriptor is an ordinary word
...
--- command -v names every reserved word
command -v if; command -v then; command -v while; command -v esac; command -v '!'; command -v '{'; command -v '}'

--- command -V says which category a name falls in
command -V if; command -V :; command -V read; command -V export

--- command describes an alias as a command line that re-creates it
alias abc='echo ABC'
c="$(command -v abc)"
unalias abc
eval "$c"
...
--- command describes a function by name
f() { :; }; command -v f; command -V f

--- command -v is silent about a name it cannot find and command -V is not
PATH= command -v _no_such_command_; echo "v=$?"; PATH= command -V _no_such_command_; echo "V=$?"

--- command -p searches the standard path when PATH is empty
PATH= command -p echo foo bar | command -p cat

--- command -pv finds a standard utility with PATH empty
PATH= command -pv cat >/dev/null; echo "v=$?"; PATH= command -pV cat >/dev/null; echo "V=$?"

--- command -V wins over -v whichever order they came in
command -v -V :; command -V -v :

--- command rejects an option it does not have
command -z cat 2>/dev/null; echo "st=$?"

--- command with nothing to run succeeds
command; echo "bare=$?"; command --; echo "sep=$?"; command -p; echo "p=$?"

--- command -- ends the options, so -v after it is a command name
command -- -v 2>/dev/null; echo "st=$?"

--- command runs an external command in a pipeline stage
echo hi | command cat

--- a function named command shadows the builtin, and only the first prefix
command() { echo FUNCTION; }; command XXX; command command echo hi

--- command in a pipeline stage bypasses a function of the same name
cat() { echo FUNCTION; }; echo hi | command cat

--- command -v describes an external command by its absolute pathname
...
--- a command substitution in an assignment prefix reaches the command
x=$(echo z) "$TESTEE" -c 'echo "[$x]"'

--- a prefix on eval is visible to the code it reads and persists after it
x=1 eval 'echo "in=[$x]"'; echo "after=[$x]"

--- a prefix on dot persists too, and unexported
x=1 . /dev/null; echo "after=[$x]"; x=2 eval 'env' | grep '^x=' ; echo "env=$?"

--- command demotes a special builtin, so its prefix is restored
a=a; a=b command :; echo "colon=[$a]"; x=1 command eval 'echo "in=[$x]"'; echo "eval=[$x]"; x=1 command . /dev/null; echo "dot=[$x]"

--- a prefix on exec with no command persists, and not under command
x=1 exec; echo "exec=[$x]"; y=1 command exec; echo "demoted=[$y]"

--- a prefix on wait is restored, wait being a regular builtin
x=1 wait; echo "after=[$x]"

--- a prefix on an external command is exported to it and not to the shell
x=$(echo z) env | grep '^x='; echo "after=[$x]"

--- a prefix in a pipeline stage is expanded in the stage
...
--- the separator precedes the operand of dot
...
--- a break before && does not run the right-hand side
for i in 1; do break && echo reached1; echo reached2; done; echo "st=$?"

--- a continue before && does not run the right-hand side
for i in 1 2; do echo "in $i"; continue && echo reached1; echo reached2; done

--- a return before && does not run the right-hand side
f() { return && echo reached1; echo reached2; }; f; echo "st=$?"

--- an and-or list reports the operand the unwind came from
f() { return 7 && echo reached; }; f; echo "st=$?"

--- an exit before && does not run the right-hand side
exit 5 && echo reached
...
--- eval on null operands reports zero
(exit 1); eval '' '' ''; echo "st=$?"

--- eval on nothing but a comment reports zero
(exit 1); eval '# nothing here'; echo "st=$?"

--- dotting a file with no commands reports zero
(exit 1); . /dev/null; echo "st=$?"

--- a dot script still sees the caller's exit status
...
--- dot searches PATH for an operand with no slash
mkdir p; printf 'exit 11\n' >p/sourced; PATH=./p; . sourced

--- dot does not fall back to the working directory
printf 'echo sourced\n' >here; PATH=/nonexistent; . here 2>/dev/null; echo "st=$?"

--- a dot script need only be readable, not executable
mkdir p; printf 'echo sourced\n' >p/readable; chmod 444 p/readable; PATH=./p; . readable

--- dot failing to find its script exits a non-interactive shell
. ./_no_such_file_ 2>/dev/null; echo not reached

--- dot failing to find its script on PATH exits a non-interactive shell too
. _no_such_file_ 2>/dev/null; echo not reached

--- a dot failure in a subshell leaves the shell that forked it running
(. ./_no_such_file_ 2>/dev/null); echo "reached st=$?"

--- a status the dot script itself reported is not a search failure
...
--- return from a dot script nested in another dot script
printf 'echo in inner\nreturn\necho not reached\n' >inner; printf 'echo in outer\n. ./inner\necho out outer\n' >outer; . ./outer; echo after

--- return from a dot script nested in a function
printf 'echo in inner\nreturn\necho not reached\n' >inner; f() { echo in f; . ./inner; echo out f; }; f; echo after

--- return from a dot script reports the status it was given
printf '(exit 1)\nreturn 17\n' >script; . ./script; echo "st=$?"

--- return out of an eval returns from the enclosing function
...
--- a return outside a function reports its operand
return 7; echo x

--- a return inside a brace group ends the input
{ return 7; echo x; }; echo y

--- a return inside a loop ends the input
while :; do return 4; done; echo x

--- a return ends a script arriving on standard input [stdin]
echo one
return 6
echo two

--- a return still returns from a function
...
--- a break with no enclosing loop does nothing
break; echo x

--- a break with no enclosing loop does nothing inside a brace group
{ break; echo x; }; echo y

--- a continue with no enclosing loop does nothing
continue; echo x

--- a break in a function does not break the caller's loop
f() { break; }; for i in 1 2; do f; echo in; done; echo out

--- a continue in a function does not continue the caller's loop
f() { continue; }; for i in 1 2; do f; echo in; done; echo out

--- a break level does not travel out of a function either
f() { for i in 1 2; do break 3; done; echo in f; }; for j in 1 2; do f; echo body; done; echo out

--- a break past the nesting depth stops at the outermost loop
for i in 1; do for j in a; do break 3; echo n1; done; echo n2; done; echo after

--- a break in a command substitution unwinds only the substitution
for i in 1; do echo "[$(break; echo insub)]"; done

--- a break in a subshell unwinds only the subshell
for i in 1; do (break; echo insub); echo body; done; echo after

--- a break in a dot script breaks the caller's loop
printf 'echo lib\nbreak\necho not reached\n' >lib; for i in 1 2; do . ./lib; echo body; done; echo after

--- a break in a dot script with no loop around it does nothing
printf 'echo lib\nbreak\necho lib end\n' >lib; . ./lib; echo "caller $?"

--- a break in the condition of a while loop leaves the loop
...
--- the verbose option echoes a dot script [stdin]
set -v
printf 'echo one\nexit 3\n' >script
. ./script

--- the verbose option does not echo an eval operand [stdin]
set -v
eval 'echo hi'
echo done
...
--- test -nt and -ot with both operands missing are false, which dash agrees with
...
--- division by zero is an expansion error
echo $((1/0)); echo st=$?

--- a malformed arithmetic expression is an expansion error
echo $((--)); echo st=$?

--- an arithmetic error in the middle of a word stops the whole command
echo A$((1/0))B; echo st=$?

--- an arithmetic error in an assignment leaves the variable alone
x=$((1/0)); echo "st=$? x=$x"

--- an arithmetic error in a special builtin's operand stops the shell
: $((1/0)); echo reached

--- a short-circuited operand that will not parse is still an error
echo $(( 0 && + )); echo st=$?

--- arithmetic that evaluates is untouched by any of this
...
--- a syntax error inside a command substitution stops the shell [stdin]
echo before
echo $(if true)
echo reached

--- an unexpected token inside a command substitution stops the shell [stdin]
echo before
echo $(echo ;;)
echo reached

--- a syntax error inside a command substitution in an assignment stops the shell [stdin]
echo before
v=$(if true)
echo reached

--- a syntax error inside a BACKQUOTED substitution stops the shell [stdin]
echo before
echo `if true`
echo reached
...
--- a runtime syntax error inside a substitution is the substitution's status, not the shell's
x=$(eval "if true"); echo "st=$? x=[$x]"; echo reached

--- a command substitution that exits non-zero does not stop anything
echo "[$(exit 7)]"; echo st=$?

--- a command substitution running a missing command does not stop anything
...
--- nounset in a for loop's word list stops the shell
set -u; for i in $nope; do echo x; done; echo reached

--- nounset in a case subject stops the shell
set -u; case $nope in *) echo x;; esac; echo reached

--- a case pattern is still not pathname-expanded
case a in *) echo star;; esac

--- nounset leaves a defaulted case subject alone
...
--- the integer limits are test operands rather than overflows
...
--- a test operand past the integer limit is a usage error rather than a saturated comparison
...
--- a signal number too large to be one is refused rather than overflowed
...
--- kill refuses a signal number too large to be one
...
--- a redirection fd too large to be one is refused
echo hi >&99999999999999999999

--- an input redirection fd too large to be one is refused
...
--- a utility syntax error in `set` still ends the shell
echo before; set -Z 2>/dev/null; echo notreached

--- an operand that is not a number still ends the shell in `shift`
...
--- a shift count past the end still ends the shell
set -- a; echo before; shift 5 2>/dev/null; echo notreached

--- and the two ways of refusing a shift report the SAME status
set -- a; ("$TESTEE" -c 'set -- a; shift 5') 2>/dev/null; echo "past=$?"; ("$TESTEE" -c 'shift abc') 2>/dev/null; echo "bad=$?"

--- an operand that is not a NAME still ends the shell in `export`
...
--- an unknown option still ends the shell in `trap`
...
--- an operand that is not a NAME still ends the shell in `unset`
echo before; unset 1bad 2>/dev/null; echo notreached

--- and a NAME with a character that cannot be in one
...
--- unsetting a FUNCTION by a name that is not a NAME is not an error
unset -f 1bad; echo "st=$?"; echo reached

--- a shell language syntax error inside `eval` still ends the shell
echo before; eval 'if' 2>/dev/null; echo notreached

--- a dot script that cannot be found still ends the shell
...
--- an assignment prefix refused before a special builtin still ends the shell
...
--- a refused assignment prefix is reported through the command's own redirections
readonly r=1; echo before; r=2 : 2>/dev/null; echo notreached

--- and through them for an EXTERNAL command too
...
--- the refused command's redirections are performed before the diagnostic
...
--- a redirection error on the refused command still ends the shell
readonly r=1; echo before; r=2 : 2>/_lesh_no_such_dir_/x; echo notreached

--- a redirection failure on a special builtin still ends the shell
...
--- errexit inside an eval stops the eval and the shell
set -e; eval "false
echo notreached"; echo alsonotreached

--- errexit inside a dot script stops the script and the shell [stdin]
set -e
printf 'false\necho notreached\n' >script
. ./script
echo alsonotreached

--- noexec set inside an eval stops the rest of that eval
eval "set -n
echo notreached"; echo alsonotreached

--- noexec set inside a dot script stops the rest of that script [stdin]
printf 'set -n\necho notreached\n' >script
. ./script
echo alsonotreached
...
--- a return inside an eval returns from the enclosing function
f() { eval 'return 7'; echo notreached; }; f; echo "st=$? after=yes"

--- a return inside a dot script ends only that script [stdin]
printf 'return 7\necho notreached\n' >script
. ./script
echo "st=$? after=yes"
...
--- LINENO starts from one [stdin]
echo $LINENO

--- LINENO counts physical lines, blank ones included [stdin]
echo $LINENO
echo $LINENO

echo $LINENO

--- LINENO counts the physical lines a multi-line command substitution spans [stdin]
: $(echo foo
echo \
baz)
echo $LINENO

--- LINENO counts the physical lines a multi-line parameter expansion spans [stdin]
: ${foo#
bar \
baz}
echo $LINENO

--- LINENO counts the physical lines a multi-line arithmetic expansion spans [stdin]
: $((1
+ \
2))
echo $LINENO

--- LINENO in a -c string counts that string's lines
echo $LINENO
echo $LINENO

--- LINENO in a loop body reports the physical line on every iteration [stdin]
for i in 1 2 3
do
echo $LINENO
done

--- LINENO counts the lines a here-document body occupies [stdin]
cat <<END
one
two
END
...
--- LINENO through an alias reports the line the alias was invoked on [stdin]
alias a='echo $LINENO'
a

--- an explicit assignment to LINENO wins over the shell's own value [stdin]
LINENO=99
echo $LINENO

--- LINENO is readable from arithmetic [stdin]
echo $((LINENO))
echo $((LINENO))

--- LINENO is not exported to a child [stdin]
echo $LINENO
env | grep -c '^LINENO=' || echo none

--- LINENO inside a command substitution counts the outer script's lines [stdin]
echo $(echo $LINENO)
echo $LINENO
...
--- noexec set inside a brace group stops the rest of that group
{ eval "set -n"; echo notreached; }; echo alsonotreached

--- noexec set inside a function body stops the rest of that body
f() { eval "set -n"; echo notreached; }; f; echo alsonotreached

--- noexec set inside a for body stops the body and the loop
for i in 1 2 3; do eval "set -n"; echo notreached; done; echo alsonotreached

--- noexec set inside an if branch stops the rest of that branch
if :; then eval "set -n"; echo notreached; fi; echo alsonotreached

--- noexec set inside a case item stops the rest of that item
case a in a) eval "set -n"; echo notreached;; esac; echo alsonotreached

--- noexec set inside a nested body stops every body enclosing it
...
--- noexec set inside a subshell stops that subshell only
...
--- a trap that exits from inside a brace group carries its status out
trap "exit 5" USR1; { kill -USR1 $$; echo notreached; }; echo alsonotreached

--- a trap that exits from inside a function body carries its status out
trap "exit 5" USR1; f() { kill -USR1 $$; echo notreached; }; f; echo alsonotreached

--- a trap that exits from inside a loop body carries its status out
trap "exit 5" USR1; for i in 1 2; do kill -USR1 $$; echo notreached; done; echo alsonotreached

--- a trap that exits from inside an if branch carries its status out
...
--- return escapes a brace group inside a function
f() { { return 7; }; echo notreached; }; f; echo "st=$?"

--- return escapes a loop inside a function
...
--- errexit inside a brace group exits the shell
set -e; { false; echo notreached; }; echo alsonotreached

--- errexit inside a function body exits the shell
set -e; f() { false; echo notreached; }; f; echo alsonotreached

--- errexit inside a while body exits rather than iterating again
set -e; while true; do false; done; echo notreached

--- errexit does not fire on a condition an if tested
set -e; if false; then echo notreached; else echo taken; fi; echo after

--- errexit does not fire on a status an or-list tested
...
--- an empty case item body answers zero and not the previous status
...
--- a PATH search does not decide whether echo is a builtin
PATH=; echo hi; echo "st=$?"

--- a PATH search does not decide whether pwd is a builtin
