...
TEST(UiHistoryVacuumLocking, AFileIdFollowsTheInodeAndNotTheName) {
	...
	const file_id before = file_id_of_path(path);
	...
	const file_id after = file_id_of_path(path);
	ASSERT_TRUE(after.valid);
	EXPECT_FALSE(file_id_equal(before, after));
	EXPECT_TRUE(file_id_equal(after, file_id_of_path(path)));
}

TEST(UiHistoryVacuumLocking, TwoAbsentFilesAreTheSameFile) {
	// fish's rule, folded into the comparison: "the file is unchanged, or the
	// new file doesn't exist". Nothing there before and nothing there now is
	// not a change somebody made under us.
	const lesh::testing::temp_path scratch;
	const file_id nothing = file_id_of_path(scratch.file("absent"));
	EXPECT_FALSE(nothing.valid);
	EXPECT_TRUE(file_id_equal(nothing, file_id_of_path(scratch.file("also absent"))));

	write_file(scratch.file("present"), as_bytes("x"));
	EXPECT_FALSE(file_id_equal(nothing, file_id_of_path(scratch.file("present"))));
}
