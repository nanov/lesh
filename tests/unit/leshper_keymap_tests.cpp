...
// the encoding, the table, the registry, and dispatch through it.
//
// The `bind` builtin's reach across the link boundary was here and is
// `ui_bind_tests.cpp`'s: it needs `runtime/`, and nothing under `leshper/` -
// source or test - reaches outside the editor and the substrate.
