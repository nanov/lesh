...
Open on the map: a `vared` entry point ([#102](https://github.com/nanov/lesh/issues/102), the last grilling ticket); the event-loop choice itself (numbers in §4, decision with the owner); the completion engine and the prompt provider's expansion (fog); the scope-chain question inherited from map #17; job-control UI; everything in requirements §8.
