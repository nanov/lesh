...
- **F-38** Resize reflows buffer, prompt, pager correctly, including mid-edit.
  - **Amended 2026-08-27 ([#185](https://github.com/nanov/lesh/issues/185)) — a repaint REPLACES the frame on screen; it does not append one.** The blitter's repaint used to start at the surface's own origin, which assumes the cursor is at the frame's top-left. On a resize it is not: it is wherever the last frame left it, at the end of the buffer. So each resize painted a fresh prompt and buffer below the old one and erased nothing — N resizes, N+1 copies. `blitter::paint_from(start, desired, out)` takes the start placement from the caller and emits `\r`, `ESC[<start.row>A` when the row is not zero, `ESC[J`, then the paint from row 0. `paint` / `paint_into` are unchanged and still erase nothing: they are the FIRST paint of a read, where the cursor really is at the origin and the rows below belong to whatever ran last.
  - **Which row, and the terminal floor.** How far above the cursor that frame's top row sits depends on what the terminal did to the frame when it was resized, and no capability in §7's floor answers it — #97 forbids start-up queries, so this is a setting (`loop_options::assume_reflow`, default **true**) and not a probe. **Reflowing terminals** rewrap soft-wrapped rows at the new width: iTerm2, kitty, WezTerm, VTE (GNOME Terminal, Tilix), Windows Terminal, and tmux inside its own panes. For those the answer is the previous frame's INPUT laid out again at the new size — `lay_out(previous input, new columns).screen.cursor().row` — which the host can ask because it keeps that input beside the frame it painted. **Non-reflowing terminals** (xterm, macOS Terminal.app) leave the rows where they were, and the answer is the row count from before the resize, `_previous.screen.cursor().row`. Rows begun by a hard newline never merge under either model, which is why the reflow answer is a re-layout rather than arithmetic on the old row count. A wrong setting costs one frame's worth of stale rows above the repaint, not a corrupted screen.
  - **Amended 2026-08-27 ([#189](https://github.com/nanov/lesh/issues/189)) — a soft-wrapped row is painted by writing THROUGH the wrap; a hard row is positioned to.** The paragraph above assumes the terminal holds the frame as one soft-wrapped logical line and rewraps it on a resize. It did not, and could not: the blitter moved between every pair of rows with `\r` + `ESC[<n>B`, which makes each row a HARD line — the terminal clipped each of them separately on a shrink (fragments like `…/lesh/buil` left above the prompt) and never rejoined them on a grow (the previous frame's top row stayed behind, and the walk-up under-counted the rows). `lay_out` now records per row whether it begins a hard line — row zero, and every row after a U+000A — and `surface::row_starts_hard_line` carries that to the blitter, which is the only thing that can tell a wrap from a newline apart. A repaint writes the row above a soft row out to its LAST COLUMN, trailing blanks included and with no `ESC[K`, and then emits the next row's first glyph with no positioning at all: that glyph is what makes the terminal wrap, and the wrap is what makes the two rows one logical line to it. zsh and fish paint soft-wrapped rows the same way, and for the same reason. The diff path still positions absolutely, and writes through only where this update happened to rewrite the row above right up to its last column — where it did not, the terminal's own wrap flag was never disturbed. Three edge rules ride with it: never `ESC[K` while a wrap is pending (the cursor is still on the last column and the erase eats the glyph — fish's `screen.rs`, zsh's "clearing eol would be evil"), never `ESC[J` there either (fish #6951), and never write through past the frame's last row, which is what keeps the bottom-right corner from scrolling. `assume_reflow` is unchanged in meaning but no longer describes us as well as the terminal: with soft rows genuinely soft, `true` is correct by construction on a reflowing terminal, and the setting remains for the terminals that do not reflow.
